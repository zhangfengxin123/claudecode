#!/usr/bin/env python3
"""
Capture new Pardus public cases (with or without description) under strict constraints:

1) non-duplicate against existing local research summaries
2) input complete (all uploads downloaded by build_chart_evidence_batch)
3) output complete strict:
...
def load_cases_from_input_json(path: Path) -> List[Dict[str, str]]:
    ...
    for it in items:
        if not isinstance(it, dict):
            continue
        cid = it.get("case_id") or it.get("id")
...
<body>
  ...
  <div class='meta'>严格口径：strict_pass=True（输入完整 + 文档可取 + 有图 + 资产全可取）。</div>
  <div class='wrap'>
    <table>
      <thead><tr><th>#</th><th>check</th><th>case</th><th>title</th><th>has_description</th><th>plot_assets</th><th>renderable</th><th>asset_fail</th><th>input_complete</th><th>strict_pass</th><th>full report</th><th>chart evidence</th><th>pardus</th></tr></thead>
...
"""
    ...
    (run_dir / "index.html").write_text(page("Mixed Description Strict Capture - All", all_rows_html, len(rows)), encoding="utf-8")
    (run_dir / "index_ok_only.html").write_text(page("Mixed Description Strict Capture - OK Only", ok_rows_html, len(ok_rows)), encoding="utf-8")
    (run_dir / "index_strict_only.html").write_text(
        page("Mixed Description Strict Capture - Strict Pass Only", strict_rows_html, len(strict_rows)),
        encoding="utf-8",
    )


def main() -> None:
    ap = argparse.ArgumentParser(description="Strict mixed-description Pardus capture orchestrator")
    ...
    ap.add_argument("--batch-timeout-sec", type=int, default=600)
    ...
    run_dir = Path(args.out_dir) if args.out_dir else (runs_dir / f"mixed_desc_strict_capture_{utc_tag()}")
    ...
    source_name = "recent_api"
    recent_pool: List[Dict[str, str]] = []
    if args.input_json:
        source_name = str(args.input_json)
        src_cases = load_cases_from_input_json(Path(args.input_json))
        for it in src_cases:
            if it["case_id"] in existing_ids:
                continue
            recent_pool.append(it)
        print(f"[info] input_cases={len(src_cases)} filtered_new_pool={len(recent_pool)}")
    else:
    ...
    # Probe candidates for mixed-description and has-plot-assets.
    ...
    while cursor < len(picked) and batch_no < args.max_batches and len(strict_rows) < args.target:
        ...
        try:
            proc = subprocess.run(
                ...
                text=True,
                check=False,
                timeout=max(60, int(args.batch_timeout_sec)),
            )
            print(proc.stdout.strip())
        except subprocess.TimeoutExpired:
            print(f"[batch {batch_no}] timeout after {args.batch_timeout_sec}s, skip batch")
            continue
        ...
        for br in batch_rows:
            ...
            up_st, up_payload = http_get_json_with_retries(UP_API.format(case_id=cid), retries=2, timeout=12)
            if up_st != 200 or not isinstance(up_payload, list):
                base.update({"status": "failed", "error": f"uploads_http_{up_st}"})
                all_rows.append(base)
                continue
            upload_names = [str(x.get("name", "")).strip().lower() for x in up_payload if isinstance(x, dict)]
            base["has_description"] = any(x == "description.txt" or x.endswith("/description.txt") for x in upload_names)
            ...
            body += (
                "<hr/><p class='muted'>说明：严格抓取报告（混合 description + 输入完整 + 图资产完整）。"
                f"plot assets: {len(plot_assets)}; renderable: {len(renderer.fig_payload)}; asset_fail: {asset_fail}.</p>"
            )
            page = ab.page_shell(
                title=f"完整报告：{title}",
                subtitle=f"来源: /view/{cid} ｜ strict mixed-description capture",
                body_html=body,
            ...
            strict_pass = asset_fail == 0 and len(renderer.fig_payload) > 0
            row = dict(base)
            row.update(
                {
                    "status": "ok",
    ...
    final = {
        ...
        "strict_with_description": len([r for r in strict_final if r.get("has_description") is True]),
        "strict_without_description": len([r for r in strict_final if r.get("has_description") is False]),
